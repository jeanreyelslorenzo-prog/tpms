...
$writableColumns = [
    ...
    'school_id', 'school_id_code_raw', 'school_name_raw', 'district_raw', 'education_program',
...
$locationStateChanged = (string)($teacher['barangay'] ?? '') !== $data['barangay']
    || (string)($teacher['barangay_psgc_code'] ?? '') !== $data['barangay_psgc_code']
    || (string)($teacher['municipality'] ?? '') !== $data['municipality']
    || (string)($teacher['municipality_psgc_code'] ?? '') !== $data['municipality_psgc_code'];
$data['coordinate_version'] = max(1, (int)($teacher['coordinate_version'] ?? 1)) + ($locationStateChanged ? 1 : 0);
$writableColumns = array_merge($writableColumns, ['location_precision','location_verified','location_verified_at','location_verified_by','coordinate_version']);
$allowedSubjectsForSchool = teacherSubjectsForOfferings(fetchSchoolTeacherSubjectOfferings($db, $selectedSchoolId));
$allowedLegacySubjects = $selectedSchoolId === (int)($teacher['school_id'] ?? 0)
    ? array_values(array_diff(parseTeacherSubjects((string)($teacher['subjects'] ?? '')), $allowedSubjectsForSchool))
    : [];
$subjectSelection = validateTeacherSubjectSelection(
    $db,
    $selectedSchoolId,
    $_POST['subjects_selected'] ?? [],
    $allowedLegacySubjects,
    $_POST['subjects_legacy'] ?? []
);
$data['subjects'] = $subjectSelection['value'];
$errors = array_merge($errors, $subjectSelection['errors']);
$errors = array_merge($errors, validateTeacherUpdateInputFields($data, $teacher));
...
if ($errors) {
    ...
    $errorLabels = [
        ...
        'school_id' => 'School Station',
        'education_program' => 'Teacher Program',
        'specialization' => 'Specialization / Major',
        'grade_level' => 'Grade Level/s Taught',
        'subjects' => 'Subjects/s Taught',
        'confirm_password' => 'Password Confirmation',
        'als_school_year' => 'ALS Assignment School Year',
        'als_clc_ids' => 'ALS CLC Assignments',
        'primary_clc_id' => 'Primary ALS CLC',
        'profile_photo' => 'Profile Photo',
    ];
    $firstErrorField = (string)array_key_first($errors);
    $firstErrorLabel = $errorLabels[$firstErrorField] ?? 'Teacher Information';
    flash('error', $firstErrorLabel . ': ' . (string)$errors[$firstErrorField]);
    redirect($failureUrl);
}
...
try {
    ...
    if ($locationStateChanged) {
        $db->prepare("DELETE FROM route_distance_cache WHERE origin_type='teacher' AND origin_id=?")->execute([$id]);
    }
    $db->commit();
    ...
    if ($locationStateChanged) {
        logActivity('UPDATE', 'teacher_location', $id, 'Teacher PSGC barangay location changed; route cache invalidated.');
    }
